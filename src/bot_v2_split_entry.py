    ...
    async def apply_real_levels_to_split(msg_id: int, sl: Optional[float], all_tps: Dict[int, float], source: str = ""):
        ...
        if strategy == "split_both_pending":
            for key in ("worst_order_ticket", "deep_order_ticket"):
                ot = info.get(key)
                if ot:
                    await run_mt5(lambda _ot=ot, _sl=sl or 0, _tp=tp3 or 0:
                ...
                for mid, info in list(_split_orders.items()):
                    ...
                    if strategy == "split_both_pending":
                        ...
                        # ── Only worst filled, deep still pending ──
                            ...
                            # At TP1 → move worst to BE
                            if not info["only_worst_tp1_be_done"]:
                                tp1 = info.get("tp1")
                                if tp1 and cp:
                                    tp1_hit = (side == "BUY" and cp >= tp1) or (side == "SELL" and cp <= tp1)
                                    if tp1_hit:
                                        worst_actual_entry = entry_prices.get(worst_pos, worst_entry)
                                        ok = await run_mt5(lambda: set_stop_loss(worst_pos, worst_actual_entry))
                                        if ok:
                                            info["only_worst_tp1_be_done"] = True
                                            log(f"TP1 BE TRIGGER (only worst filled): price ${cp:.2f} hit TP1 ${tp1:.2f}", "INFO")
                                            log(f"  Worst position {worst_pos} SL → BE @ ${worst_actual_entry:.2f}", "INFO")
                            ...
                            if not info["only_worst_tp1_be_done"]:
                                ...
                                if tp1 and cp:
                                    ...
                                    if tp1_hit:
                                        ...
                                        if ok:
                                            info["only_worst_tp1_be_done"] = True
                                            log(f"TP1 BE: market pos {market_pos} SL → ${market_actual_entry:.2f}", "INFO")
                ...
                for ticket in active_tickets:
                    ...
                    if outcome_tracker.is_shadow_tracking(ticket):
                        if outcome_tracker.should_finalize(ticket):
                            ...
                            log(f"OUTCOME TRACKER: Ticket {ticket} shadow FINALIZED | P&L=${profit:.2f} | Reason: {reason}", "INFO")
                    ...
                    if not pos:
                        ...
                        log(f"OUTCOME TRACKER: Ticket {ticket} CLOSED | P&L=${final_profit:.2f} | "
                            f"Close=${close_price:.2f} | Seq: {' → '.join(seq) or 'NONE'}", "INFO")
