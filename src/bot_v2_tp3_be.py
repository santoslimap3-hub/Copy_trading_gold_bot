#!/usr/bin/env python3
"""
Gold Trading Bot - TP3 Target + Breakeven on TP1 Strategy (Robust Edition)
Connects to Telegram channel for gold trading signals and executes trades automatically.

STRATEGY:
  1. Signal received → Enter immediately with failsafe SL ($8) and failsafe TP ($8 above zone)
  2. Wait for message edit with real SL and TP values
  3. Parse TP levels and SL from the edited message; set real TP to TP3 (fallback TP2, TP1)
  4. When price crosses TP1, move stop loss to breakeven (entry price)
  5. Let the broker handle final exit via SL/TP.
...
SESSION_FILE = "trading_bot_session_tp3be"
...
FAILSAFE_TP_DISTANCE = 8.0  # failsafe TP: $8 away from entry (until real TP arrives)
FAILSAFE_TP_ABOVE_ZONE = 8.0  # failsafe TP: $8 beyond worst zone edge
...
_be_moved: Set[int] = set()              # tickets where SL has already been moved to breakeven
...
def select_target_tp(all_tps: Optional[Dict[int, float]]) -> Optional[float]:
    """Return TP3 if available, else TP2, else TP1, else None.
    all_tps is a dict like {1: 4877.0, 2: 4875.0, 3: 4873.0}."""
    if not all_tps:
        return None
    for level in (3, 2, 1):
        if level in all_tps and all_tps[level]:
            return float(all_tps[level])
    return None
...
def print_bot_status():
    ...
    log(f"  Strategy: {metrics['entry_strategy']} (TP3 TARGET + BREAKEVEN ON TP1)", "INFO")
...
def execute_signal_trade(side: str, msg_id: int, msg_text: str):
    ...
    if ticket:
        log(_LOG_SEP, "INFO")
        log(f"POSITION OPENED (TP3 TARGET + BREAKEVEN ON TP1)", "INFO")
...
def execute_limit_trade(side: str, msg_id: int, msg_text: str, limit_price: float, zone_low: float, zone_high: float):
    ...
    if price_in_zone:
        ...
        if side == "BUY":
            failsafe_sl = entry_price - FAILSAFE_SL_DISTANCE
            failsafe_tp = zone_high + FAILSAFE_TP_ABOVE_ZONE
        else:
            failsafe_sl = entry_price + FAILSAFE_SL_DISTANCE
            failsafe_tp = zone_low - FAILSAFE_TP_ABOVE_ZONE
...
async def main():
    ...
    log(f"  Failsafe TP above zone: ${FAILSAFE_TP_ABOVE_ZONE:.2f}", "INFO")
    log(f"  Strategy: TP3 TARGET + BREAKEVEN ON TP1", "INFO")
    ...
    async def update_position_sl_tp(ticket: int, sl: float = None, tp1: float = None, source: str = "", all_tp_levels: Dict[int, float] = None):
        """Update SL and real TP on a position. TP target = TP3 > TP2 > TP1."""
        ...
        # Prefer TP3 > TP2 > TP1 from all available levels
        target_tp = select_target_tp(all_tp_levels) or tp1
        new_sl = sl if sl else current_sl
        new_tp = target_tp if target_tp else current_tp
        ...
        if target_tp:
            if side == "BUY" and target_tp <= entry:
                log(f"Target TP ${target_tp:.5f} is below entry ${entry:.5f} for BUY - ignoring TP update", "WARN")
                new_tp = current_tp
            elif side == "SELL" and target_tp >= entry:
                log(f"Target TP ${target_tp:.5f} is above entry ${entry:.5f} for SELL - ignoring TP update", "WARN")
                new_tp = current_tp
        ...
        log("=" * 70, "INFO")
        log(f"UPDATING REAL SL/TP ON TICKET {ticket}{source}", "INFO")
        ...
        log(f"Ticket {ticket} updated with real SL and target TP. Broker handles exit.", "INFO")
    ...
    async def process_message(text: str, msg_id: int, channel_name: str, source: str = ""):
        """
        Core message processing - TP3 Target + Breakeven on TP1 Strategy with Zone-Aware Limit Orders.
            ...
            try:
                ...
                for ticket in active_tickets:
                    ...
                    for level in newly_hit:
                        ...
                        # ── Breakeven on TP1 ──
                        # Real open position only (not virtual/shadow), not already moved
                        if (level == "TP1"
                                and not info.get("virtual")
                                and not info.get("position_closed")
                                and ticket not in _be_moved):
                            entry_p = entry_prices.get(ticket, info.get("entry_price", 0))
                            if entry_p > 0:
                                log(f"TP1 HIT on ticket {ticket} → moving SL to breakeven (${entry_p:.5f})", "INFO")
                                be_ok = await run_mt5(lambda t=ticket, ep=entry_p: set_stop_loss(t, ep))
                                if be_ok:
                                    _be_moved.add(ticket)
                                    log(f"Breakeven set for ticket {ticket}", "INFO")
                                else:
                                    log(f"Failed to set breakeven for ticket {ticket}", "WARN")
